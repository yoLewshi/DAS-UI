...
function install_das-ui_docker {
    ...
    sudo docker run -d -p 80:3000 --restart=always --name=das-ui --net das-net --ip 172.18.0.7 openrvdas/das-ui-image && sudo docker image prune --all -f --filter label=openrvdas="das-ui" && sudo docker volume prune --all -f --filter label=openrvdas="das-ui"
}
